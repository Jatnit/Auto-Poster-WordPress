...
| `src/wp_auto_poster/web/app_factory.py` | Flask app factory | Creates the Flask app and registers route modules. |
| `src/wp_auto_poster/web/routes.py` | HTTP API routes | Routes parse requests and delegate to runtime callbacks. Response shapes stay compatible. |
...
| `src/wp_auto_poster/content/generation.py` | Provider router | Normalizes provider calls through a shared generation entrypoint. |
...
| `src/wp_auto_poster/state/app_state.py` | Runtime state model | Centralizes progress, generated content, queues, image usage, and control flags. |
...
| `src/wp_auto_poster/utils/logging.py` | UI/system logging helpers | Keeps log formatting and checklist event creation consistent. |
